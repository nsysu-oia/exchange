<template>
  ...
  <div class="export" :style="{ width: sectionWidth }">
    <p><b>獎助證明書不需提交至國際處，如有需要可自行匯出儲存</b></p>
    <button
      type="button"
      class="button"
...
import questions from '@/assets/contents/proof-of-scholarship.yaml'
function resize() {
...
export default {
  ...
  created() {
    ...
    axios.get('//' + backendHost + ':3000/proof-of-scholarship').then(({ data }) => {
  ...
  methods: {
    ...
    syncDB(e) {
      ...
      axios.post('//' + backendHost + ':3000/proof-of-scholarship', { values })
    },
    validate(e) {
      const id = e.target.id
