<template>
  <div id='window'>
    <h1>上傳{{ item.title }}</h1>
    <input id="file" type="file" :multiple="!!item.multiple" @change="filesSelected">
    ...
    upload (e) {
      ...
      if (this.item.multiple) {
        ...
        for (let i = 0; i < this.files.length; i++) {
          ...
          formData.append('filename', parseInt(i) + '.' + this.files[i].name.split('.').slice(-1)[0])
